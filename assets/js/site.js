(function () {
  ...
  function renderRepeats() {
    document.querySelectorAll('[data-repeat]').forEach(function (container) {
      ...
      values.forEach(function (item) {
        ...
        bind(node, item);
        node.querySelectorAll('[data-repeat]').forEach(function (nested) { var nestedValues = get(item, nested.getAttribute('data-repeat')) || [], nestedTemplate = nested.firstElementChild; nested.innerHTML = ''; nestedValues.forEach(function (value) { var child = nestedTemplate.cloneNode(true); if (child.matches('[data-bind]')) child.textContent = displayValue(value); child.querySelectorAll('[data-bind]').forEach(function (element) { element.textContent = displayValue(value); }); nested.appendChild(child); }); });
  ...
  function renderDetail() {
    ...
    if (type === 'service') {
      target.innerHTML = '<div class="detail-hero"><div><span class="eyebrow"><span class="sq"></span>' + esc(item.number) + ' — Service</span><h1>' + esc(item.title) + '</h1><p class="detail-hero lead">' + esc(item.short) + '</p><div class="detail-meta"><h3>Capabilities</h3><ul class="taglist">' + (item.tags || []).map(function (tag) { return '<li>' + esc(displayValue(tag)) + '</li>'; }).join('') + '</ul></div></div><figure class="detail-image"><img src="' + esc(item.image) + '" alt=""><figcaption>' + esc(item.figureCaption || '') + '</figcaption></figure></div><div class="detail-copy"><p>' + esc(item.description) + '</p></div>' + (item.stats && item.stats.length ? '<div class="assets"><div class="assets__head"><h4>Reference material on hand</h4></div><div class="stat-grid">' + item.stats.map(function (stat) { return '<div class="stat"><b>' + esc(stat.value) + '</b><span>' + esc(stat.label) + '</span></div>'; }).join('') + '</div></div>' : '') + '<div class="detail-meta"><h3>Notes</h3>' + (item.notes || []).map(function (note) { return '<p>' + esc(note) + '</p>'; }).join('') + '</div>';
    } else {
